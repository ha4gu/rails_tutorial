...
class UsersControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should redirect from edit when logged in as wrong user" do
  ...
  test "should redirect from update when logged in as wrong user" do
    log_in_as(@other_user)
    get edit_user_path(@user)
    patch user_path(@user), params: { user: { name: @user.name,
      email: @user.email } }
  ...
  test "should redirect from destroy when logged in as a non-admin" do
    log_in_as(@other_user)
    assert_no_difference 'User.count' do
      delete user_path(@user)
    end
